        ...
        if (validationmelength == 0 && validationMessages2.length == 0) {
            CommonAjaxFunction('GET', '/SchoolReport/ReportOrganisationFeeSummary', null, function (response) {
                ...
                CommonAjaxFunction('GET', '/SchoolReport/FeeEntityInstanceSummaryone', orgfeesummary, function (response) {
                    ...
                    bindorganisationfeesummary(response, headernames, columnnames, "#appendinsidetablemain", null, null, "bindsecondtable", 2, true, true, "first", "firsttable");
...
function bindsecondtable(id, event) {
    ...
    CommonAjaxFunction('POST', '/SchoolReport/FeeEntityInstanceSummarytwo', formData2, function (response) {
        ...
        if (removedElement.length > 0) {
            ...
            bindorganisationfeesummary(response, headernames, columnnames, null, event, "aquamarine", "bindthirdtable", 2, true, false, "second", "secondtable");
        }
        loaddingimg.css('display', 'none');
...
function bindthirdtable(id, event) {
    ...
    CommonAjaxFunction('POST', '/SchoolReport/FeeEntityInstanceSummarythree', formData2, function (response) {
        ...
        if (removedElement.length > 0) {
            ...
            bindorganisationfeesummary(response, headernames, columnnames, null, event, "aquamarine", "bindfourthtable", 2, true, false, "third", id);
        }
        loaddingimg.css('display', 'none');
...
function bindfourthtable(id, event) {
    ...
    var closestElement = $(event).closest('.third');
    //var FeeTermId = closestElement.find('input[type="text"]').val();
    //var bindfourthtableValue = closestElement.find('#bindfourthtable').val();
    var FeeTermId = closestElement.find('#TermNameid').val();//termNameidValue


    //var FeeTermId = $(event).closest('.third').closest('.second').find('table').find('#bindthirdtable').val();
    //var FeeTermId = $(event).closest('.third').find('input[type="text"]').val();
    ...
    CommonAjaxFunction('POST', '/SchoolReport/FeeEntityInstanceSummaryfour', formData2, function (response) {
        ...
        if (removedElement.length > 0) {
            ...
            bindorganisationfeesummary(response, headernames, columnnames, null, event, "aquamarine", null, 2, false, true, "fourth", "fourthtable");
        }
        loaddingimg.css('display', 'none');
...
function bindpockettable() {
    ...
    CommonAjaxFunction('GET', '/SchoolReport/PocketMoneyDetails', orgfeesummary, function (response) {
        ...
        bindorganisationfeesummary(response, headernames, columnnames, "#appendpocketmoney", null, null, "nofun", 1, false, true, "pocket", "bindpockettable");
...
function bindorganisationfeesummary(response, headernames, columnnames, appendid, event, bcolor, onclickfun, startcolunmnindex, linkexceptlastrow, linkallfalse, Class, Geetingid) {
    debugger;
    // try {
            ...
            if (linkallfalse) {
                if (i < response.length - 1) {
                    if (linkexceptlastrow) {
                        maintable += '<tr><td class="appendinsidetable" onclick="' + onclickfun + '(\'' + response[i][columnnames[1]] + '\', this)" style="color: #7367f0 !important; cursor: pointer;">' + response[i][columnnames[0]] + ' <input type="text" id="' + onclickfun + '" value="' + response[i][columnnames[1]] + '" hidden style="display:none" readonly /><input type="text" id="TermNameid" value="' + Geetingid + '" hidden style="display:none" readonly /></td>';
                        /*maintable += '<tr><td class="appendinsidetable" onclick="' + onclickfun + '(\'' + response[i][columnnames[1]] + '\', this)" style="color: #7367f0 !important; cursor: pointer;">' + response[i][columnnames[0]] + ' <input type="text" id="' + onclickfun + '" value="' + response[i][columnnames[1]] + '" hidden style="display:none" readonly /></td>';*/
