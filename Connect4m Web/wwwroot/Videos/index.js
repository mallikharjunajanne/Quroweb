export const importFileandPreview = (file, revoke)
    => {
        ...
        {
            window.URL = window.URL || window.webkitURL; let preview = window.URL.createObjectURL(file);
            if (revoke)
            { window.URL.revokeObjectURL(preview) }
            setTimeout(() => { resolve(preview) }, 100)
        })
}; export const generateVideoThumbnails = async (videoFile, numberOfThumbnails, type, cb) =>
{
    let thumbnail = [];
    let fractions = [];
    return new Promise(async (resolve, reject) => {
        var _a; if (type === "file")
        {
            ...
            let promiseArray = fractions.map(async (time, index) => {
                ...
                res.forEach(res => { thumbnail.push(res) });
                resolve(thumbnail)
            }).catch(err => { reject(err) }).finally(() => resolve(thumbnail))
        }).catch(err => { reject(err) }); reject("something went wrong")
    })
}; const getVideoThumbnail = (file, videoTimeInSeconds) =>
{
    return new Promise((resolve, reject) =>
    {
        var _a; if ((_a = file === null || file === void 0 ? void 0 : file.type) === null || _a === void 0 ? void 0 : _a.match("video"))
        {
            importFileandPreview(file).then(urlOfFIle => { getVideoCover(urlOfFIle, videoTimeInSeconds).then(res => { resolve(res) }) })
        } else if (file)
        { getVideoCover(file, videoTimeInSeconds).then(res => { resolve(res) }).catch(err => { reject(err) }) }
        else { reject("file not valid") }
    })
                ...
                videoPlayer.addEventListener("seeked", () => {
                    ...
                    ctx.canvas.toBlob(blob => {
                        ...
                        reader.onloadend = function () {
                            var base64data = reader.result; resolve(base64data)
                        }
                    }, "image/jpeg", 1)
                })
            })
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            var timeupdate = function () {
                if (snapImage()) {
                    video.removeEventListener("timeupdate", timeupdate); video.pause()
                }
                ...
                }, "image/jpeg", 1); var success = (image === null || image === void 0 ? void 0 : image.length) > 1e5; if (success) { URL.revokeObjectURL(urlOfFIle); resolve(image) } return success
            }; video.addEventListener("timeupdate", timeupdate); video.preload = "metadata"; video.src = urlOfFIle; video.muted = true; video.playsInline = true; video.crossOrigin = "Anonymous"; video.currentTime = videoTimeInSeconds; video.play().then().catch(err => { reject({ status: 500, reason: `Access to video at ${urlOfFIle} from origin ${window.location.hostname} has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.`, message: err }) })
        } catch (error) { reject(error) }
    })
        ...
        var _a; try {
            if (videoFile) {
                if ((_a = videoFile === null || videoFile === void 0 ? void 0 : videoFile.type) === null || _a === void 0 ? void 0 : _a.match("video"))
                {
                    ...
                    { generateVideoDurationFromUrl(url).then(res => { resolve(res) }) })
                } else { generateVideoDurationFromUrl(videoFile).then(res => { resolve(res) }) }
            } else { reject("Cannot generate video duration for this video file.") }
        } catch (error) { reject(error) }
    })
}; const generateVideoDurationFromUrl = url => { return new Promise((resolve, reject) => { let video = document.createElement("video"); video.addEventListener("loadeddata", function () { resolve(video.duration); window.URL.revokeObjectURL(url) }); video.preload = "metadata"; video.src = url; video.muted = true; video.crossOrigin = "Anonymous"; video.playsInline = true; video.play() }) };